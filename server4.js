...
app.patch('/updateData/:NISN', (req, res) => {
  const requestedNISN = req.params.NISN;
  ...
  if (dataIndex !== -1) {
    ...
    const updatedFields = Object.keys(newData).filter(key => oldData[key] !== newData[key]);

    if (updatedFields.length === 0) {
      // Tidak ada perubahan, berikan respons sesuai
      return res.json({ message: 'No changes detected', updatedFields: [] });
    }
    ...
    db.query('UPDATE daftar_pd SET ? WHERE NISN = ?', [req.data[dataIndex], requestedNISN], (err) => {
      ...
      db.query('INSERT INTO log_updates SET ?', logEntry, (logErr) => {
        if (logErr) {
          ...
          // Tidak menghentikan respons jika terjadi kesalahan log
        }
      });
